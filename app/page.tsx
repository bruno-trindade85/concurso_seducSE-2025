...
type SortKey = "school" | "category" | "area" | "disciplines" | "possession" | "situation";
...
const directorates = ["DEA", "DRE 1", "DRE 2", "DRE 3", "DRE 4", "DRE 5", "DRE 6", "DRE 7", "DRE 8"];
...
export default function Home() {
  ...
  const schools = useMemo(
    () => [...new Set(professionals.map((person) => person.school))].sort((a, b) => a.localeCompare(b, "pt-BR")),
    [],
  );

  const schoolScope = useMemo(
    () => professionals.filter((person) => school === "Todas" || person.school === school),
    [school],
  );
  ...
  const pendingDates = filtered.filter((person) => person.possession === "Não informada").length;
  ...
  const selectedCity = school === "Todas"
    ? "6 municípios · 10 escolas"
    : professionals.find((person) => person.school === school)?.city ?? "";
        ...
        <div className="dashboard">
          ...
          <section className="filter-panel" aria-label="Filtros do dashboard">
            ...
            <label className="school-filter"><span>Escola</span><select value={school} onChange={(e) => handleSchoolChange(e.target.value)} aria-label="Selecionar escola">
              <option value="Todas">Todas as escolas — consolidado DRE 5</option>
          ...
          <section className="analytics-grid">
            ...
            <article className="card">
              <CardHeader title="Profissionais por vínculo" subtitle={school === "Todas" ? "Consolidado das dez escolas" : "Quadro da escola selecionada"} />
            ...
            <div className="table-wrap">
              <table>
                <thead><tr>
                  ...
                  <SortHeader label="Data de posse" sortKey="possession" activeKey={sortKey} direction={sortDirection} onSort={handleSort} />
...
function sortValue(person: Professional, key: SortKey): string | number | null {
  ...
  const match = person.possession.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
